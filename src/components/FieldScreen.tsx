...
export const FieldScreen = ({ state, goToScreen }: Props) => {
  ...
  const advance = () => { if (!activeNPC) return; if (idx < activeNPC.dialogue.length-1) setIdx(idx+1); else { setActiveNPC(null); setIdx(0); } };
  const party = [...(state.player?[{name:state.player.name}]:[]), ...state.party.filter(m=>m.joined).map(m=>({name:m.name}))];
  return (
