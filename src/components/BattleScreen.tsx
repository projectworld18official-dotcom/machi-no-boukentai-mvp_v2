...
export const BattleScreen = ({ state: _state, goToScreen }: Props) => {
  ...
  const handleCommand = (cmd: typeof COMMANDS[number]) => {
    if (cmd === 'たたかう') playSE('attack');
    else if (cmd === 'まほう') playSE('magic');
    setMessage(`${cmd} を えらんだ！`);
  };
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', minHeight:'100dvh', background:'#1a1a2e', padding:16 }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, marginBottom:12 }}>
        {COMMANDS.map(cmd => (
          <button key={cmd} onClick={() => handleCommand(cmd)} style={{ padding:14, background:'#16213e', border:'1px solid #f0d080', color:'#f0d080', borderRadius:4, cursor:'pointer', fontSize:16, fontFamily:'inherit' }}>{cmd}</button>
        ))}
      </div>
